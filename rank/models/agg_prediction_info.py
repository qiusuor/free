...
def agg():
    ...
    for label in tqdm(os.listdir(EXP_DIR)):
        ...
        for exp_config in  os.listdir(label_dir):
            ...
            agg_result["all_exp"][exp_config] = dict()
            agg_result["all_exp"][exp_config]["label"] = label
            agg_result["all_exp"][exp_config]["avg_epoch"] = avg_epoch
            agg_result["all_exp"][exp_config]["avg_ap"] = avg_ap
            agg_result["all_exp"][exp_config]["avg_auc"] = avg_auc
            agg_result["all_exp"][exp_config]["agv_high"] = agv_high
            agg_result["all_exp"][exp_config]["avg_low"] = avg_low
            agg_result["all_exp"][exp_config]["avg_sharp"] = avg_sharp
    ...
    agg_result["all_exp"] = sorted(agg_result["all_exp"].items(), key=lambda x:x[1]["avg_sharp"])
