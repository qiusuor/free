...
def train_lightgbm(argv):
    ...
    if pred_mode:
        train_dataset[["code", "code_name", "pred", label, f"y_next_{n_day}_d_close_high_ratio", "y_next_1d_close_2d_open_rate", f"y_next_{n_day}_d_high_ratio", f"y_next_{n_day}_d_low_ratio", "price"]].to_csv(os.path.join(save_dir, "train_set_EPOCH_{}_AP_{}_AUC_{}.csv".format(epoch, train_ap, train_auc)))
    ...
    res_val = val_dataset[["code", "code_name", "pred", label, f"y_next_{n_day}_d_close_high_ratio", "y_next_1d_close_2d_open_rate", f"y_next_{n_day}_d_high_ratio", f"y_next_{n_day}_d_low_ratio", "price"]]
    meta = dict()
    meta["config"] = {
        "label": label,
        "train_len": train_len,
    ...
    meta["daily"] = dict()
    watch_list = [f"y_next_{n_day}_d_high_ratio", f"y_next_{n_day}_d_low_ratio", "y_next_1d_close_2d_open_rate", f"y_next_{n_day}_d_close_high_ratio"]
